...
const categories = [
  { categoria: "AjuntamentDeBarcelona", count: 12 },
];

let globalIndex = 1; // Índice acumulativo para las imágenes
const initialPosts = categories.flatMap(({ categoria, count }) =>
  Array.from({ length: count }, () => ({
    img: `/image/blogs/onrentarse/blog${globalIndex++}.jpg`, // Incrementar el índice acumulativo
    title: "",
    direccion: "",
    metro: "",
    bus: "",
      ...
      try {
        ...
        for (const id of idsToFetch) {
          const [
            ...
            axios.get(`http://localhost:5000/api/onrentarse/nombre/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/direccion/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/metro/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/bus/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/numero/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/email/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/horario/${id}`),
            axios.get(`http://localhost:5000/api/onrentarse/condicio/${id}`),
          ]);
